...
main() {
  ...
  echo "phase-ukb-200k started on $(date)"
  ...
  in_dir="${project}:/Bulk/Whole genome sequences/Population level WGS variants, pVCF format - interim 200k release/"
  ...
  function filter () {
    ...
    in_vcf_args=$( seq ${start_block} ${end_block} | sed -e "s!^!-i \"in_vcf=\${in_dir}ukb24304_c${chrom}_b!" -e "s!\$!_v1.vcf.gz\"!" | tr '\n' ' ')
  ...
  echo "phase-ukb-200k finished on $(date)"
}
